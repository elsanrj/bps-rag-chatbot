...
def detect_abstract_type(abstract: str) -> tuple[str, str | None]:
    ...
    if a_tags:
        for a in a_tags:
            ...
            if "selanjutnya" in a_text or "selengkapnya" in a_text:
...
def convert_type_a(abstract: str) -> str:
    ...
    Handle dua format: HTML berpoin (<ul><li>) dan paragraf deskriptif.

    Format HTML berpoin:
      Setiap <li> dikonversi menjadi kalimat diakhiri titik.

    Format paragraf:
      Langsung strip HTML dan normalisasi whitespace.
    """
    soup = BeautifulSoup(abstract, "html.parser")

    # Cek apakah ada struktur list
    ...
    # Paragraf deskriptif — strip semua tag
    return normalize_whitespace(clean_html(abstract))
...
def scrape_external_abstract(url: str) -> tuple[str, str | None]:
    ...
    try:
        response = requests.get(
            ...
            headers={"User-Agent": "Mozilla/5.0 (compatible; BPS-RAG-Bot/1.0)"},
        )
    ...
    for selector in _BPS_ABSTRACT_SELECTORS:
        ...
        if element:
            ...
            if text and len(text) > 50:   # minimal 50 karakter agar tidak ambil noise
                abstract_text = text
    ...
    if not abstract_text:
        ...
        if paragraphs:
            ...
            text = normalize_whitespace(longest.get_text(separator=" "))
            if len(text) > 50:
                abstract_text = text
                logger.info(f"  Abstrak fallback (paragraf terpanjang): {len(text)} karakter")
    ...
    for selector in _BPS_PDF_SELECTORS:
        ...
        if pdf_element:
            ...
            if href:
                ...
                pdf_url = href
                logger.info(f"  PDF URL ditemukan: {pdf_url}")
                break
...
def process_one_brs(raw_doc: dict) -> dict | None:
    ...
    # Deteksi region dari content — khusus BRS: Kota Bandung / Jawa Barat / Nasional
    region = resolve_region_brs(content)
